...
fn main() -> Result<()> {
    ...
    if args.len() <= 1 || args[1] == "help" {
        ...
        let raw_tx = &args[2];
        let out_index: u32 = args[3].parse().map_err(|e| anyhow::anyhow!("Invalid out_index: {}", e))?;
        let out_addr = &args[4];
        let amount: u64 = args[5].parse().map_err(|e| anyhow::anyhow!("Invalid amount: {}", e))?;
        let fee_rate: f64 = args[6].parse().map_err(|e| anyhow::anyhow!("Invalid fee_rate: {}", e))?;
